  ...
  return (
    <div className={cn('scroll-x rounded-lg border border-hairline bg-void p-3', className)}>
      ...
      >
        ...
        <text x="240" y="60" fill="#38BDF8" fontSize="10" fontWeight="600">
          ML INFERENCE PATH
        </text>
        <text x="240" y="180" fill="#22C55E" fontSize="10" fontWeight="600">
          PHYSICS PATH
        </text>

        {/* Frontend */}
        <Box x={20} y={140} w={150} h={82} title="React frontend" accent="#8B5CF6"
          lines={['TypeScript · Vite', 'TanStack Query', '3Dmol.js viewer']} />

        {/* API */}
        <Box x={196} y={140} w={0} h={0} title="" accent="#000" lines={[]} />
        <rect x="186" y="150" width="34" height="62" rx="6" fill="#111936" stroke="#1E2A4A" />
        <text x="203" y="176" fill="#94A3B8" fontSize="9" textAnchor="middle">API</text>
        <text x="203" y="188" fill="#64748B" fontSize="8" textAnchor="middle">v1</text>

        {/* ML lane */}
        <Box x={244} y={78} w={128} h={70} title="Model loader" accent="#38BDF8"
          lines={['load once', 'SHA-256 verify', 'schema verify']} />
        <Box x={392} y={78} w={128} h={70} title="Featurisation" accent="#38BDF8"
          lines={['reference table', 'or recomputed', 'candidate ranking']} />
        <Box x={540} y={78} w={124} h={70} title="Inference" accent="#38BDF8"
          lines={['unknown-cat guard', 'envelope check', 'aggregate + warn']} />

        {/* Physics lane */}
        <Box x={244} y={198} w={128} h={70} title="Job manager" accent="#22C55E"
          lines={['worker thread', 'atomic status.json', 'one job at a time']} />
        <Box x={392} y={198} w={128} h={70} title="OpenMM engine" accent="#22C55E"
          lines={['amber14 + GBn2', 'Langevin, fixed seed', 'step-driven progress']} />
        <Box x={540} y={198} w={124} h={70} title="Analysis" accent="#22C55E"
          lines={['RMSD · RMSF · Rg', 'energy · temperature', 'drift proxy']} />

        {/* Convergence */}
        <Box x={694} y={140} w={162} h={82} title="Compare + report" accent="#F59E0B"
          lines={['ML vs proxy', 'labelled separately', 'JSON + CSV export']} />

        {/* Storage */}
        <rect x="244" y="304" width="420" height="52" rx="8" fill="#0B1024" stroke="#1E2A4A" strokeDasharray="4 3" />
        <text x="454" y="324" fill="#94A3B8" fontSize="10" textAnchor="middle" fontWeight="600">
          Local filesystem — no database
        </text>
        <text x="454" y="340" fill="#64748B" fontSize="8.5" textAnchor="middle" fontFamily="monospace">
        ...
        <text x="435" y="256" fill="#64748B" fontSize="8.5" textAnchor="middle">
...
function Box({
  ...
  return (
    <g>
      ...
      <text x={x + 12} y={y + 20} fill="#F8FAFC" fontSize="11" fontWeight="600">
        {title}
      </text>
